// simul-markets-sbtc-stx-jing-swap-deposit-y.js
// Stxer simulation: atomic taker swap on sbtc-stx market — symmetric to
// simul-markets-sbtc-stx-jing-swap.js but with `deposit-x = false`, so
// the taker deposits STX (token-y) and consumes pre-staged sBTC (token-x).
//
// Run: npx tsx simulations/simul-markets-sbtc-stx-jing-swap-deposit-y.js
import {
  uintCV,
  contractPrincipalCV,
  stringAsciiCV,
...
async function main() {
  ...
  console.log("\n=== MARKETS-SBTC-STX-JING ATOMIC SWAP (deposit-x=false) ===");
  console.log("sBTC pre-stages 100k sats, then STX depositor calls swap(deposit-x=false).");
  console.log("Atomic: deposit-token-y + close + settle-with-refresh (BTC/USD + STX/USD).\n");
  ...
  const sessionId = await sim
    ...
    // Atomic swap: STX depositor takes the sBTC liquidity in one tx.
