// verify-vault-sbtc-stx-v5-parked.js
// Self-verifying stxer mainnet-fork harness for vault-sbtc-stx-v5 against
// the LIVE markets-sbtc-stx-jing-v5 / jing-core-v4 / jing-vault-auth, with
...
//
// Run: PYTH_API_KEY=... npx tsx simulations/verify-vault-sbtc-stx-v5-parked.js
...
const MARKET_ID = `${CHAVITA}.markets-sbtc-stx-jing-v5`;
const CORE_ID = `${CHAVITA}.jing-core-v4`;
const VAULT_NAME = "vault-sbtc-stx-v5";
...
async function main() {
  console.log("=== vault-sbtc-stx-v5 PARKED-ORDER harness (live v5 stack) ===\n");
  ...
  // ---- P0 clear the live book ----
  for (const y of LIVE_Y) tx(`P0 cancel live bid ${y.slice(0, 6)}`, call(y, MARKET_ID, "cancel-token-y-deposit", [wstxTrait, wstxAsset]), (v) => v.startsWith("(ok"));
  for (const x of LIVE_X) tx(`P0 cancel live ask ${x.slice(0, 6)}`, call(x, MARKET_ID, "cancel-token-x-deposit", [sbtcTrait, sbtcAsset]), (v) => v.startsWith("(ok"));
  ...
  tx("V1 deploy vault v5 (source as is, live refs)", (b) => b.withSender(OWNER).addContractDeploy({ contract_name: VAULT_NAME, source_code: vaultSrc, clarity_version: ClarityVersion.Clarity5 }), "(ok true)");
  tx("V1 chavita verifies the vault hash in jing-core-v4", call(CHAVITA, CORE_ID, "set-verified-contract", [vaultCV]), "(ok true)");
  ...
  FILLERS.forEach((f, i) => {
    ...
    tx(`V4 filler ${i + 1} rests 2000 at +5%`, call(f, MARKET_ID, "deposit-token-x", [uintCV(FILL), uintCV(ASK_NEAR), UPD, sbtcTrait, sbtcAsset]), `(ok u${FILL})`);
  });
  ...
  tx("V4 parker rests 2000 in range (-10%) with a fresh update -> parks the farthest ask", call(PARKER, MARKET_ID, "deposit-token-x", [uintCV(FILL), uintCV(ASK_IN), UPD, sbtcTrait, sbtcAsset]), `(ok u${FILL})`);
