...
// changed. Generated from verify-markets-v4-remainder-cross.js.
...
// Everything below is the v2 harness otherwise. Run: PYTH_API_KEY=<key> npx tsx simulations/verify-markets-v6-remainder-cross.js
...
const CORE = "jing-core-v5"; // the market binds .jing-core-v5
...
async function main() {
  ...
  const X1_AMT = 2000n;
  const M2_AMT = 3000n;
  const M3_AMT = 8000n; // v4: sized for the live mid (~331 sats/STX) so S7a's walk stays inside M3
  ...
  const repriceY = (sender, limit) =>
    call(sender, "reprice-or-swap-token-y", [uintCV(limit), noneCV(), DUMMY_VAA, sbtcTrait, sbtcAsset, wstxTrait, wstxAsset]);
  const repriceX = (sender, limit) =>
    call(sender, "reprice-or-swap-token-x", [uintCV(limit), noneCV(), DUMMY_VAA, sbtcTrait, sbtcAsset, wstxTrait, wstxAsset]);
