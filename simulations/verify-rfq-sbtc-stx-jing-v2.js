...
// quoted-out +/- 20bps drift, TCA reference benchmark (fresh <120s), MM
// whitelist. See contracts/v2-without-pyth/README.md for the design.
//
...
const MAX_PREMIUM_BPS = 2000n;           // signed tolerance vs native band
...
async function main() {
  ...
  const cLow = (mid * 7500n) / 10000n;                     // breaches 20% floor
  const cHigh = (mid * 12500n) / 10000n;                   // breaches 20% ceiling
  ...
  const sig = (rfqId, quotedOut, over = {}) => signIntent(
    buildRfqAuthHashHexV2({
      ...
      refVenue: REF_VENUE, maxPremiumBps: MAX_PREMIUM_BPS,
  ...
  evalc("build-auth-hash parity",
    `(build-auth-hash u0 '${MM} u${cOk} u${nativePrice} u${refOk} "${REF_VENUE}" u${MAX_PREMIUM_BPS} u${AUTH_BIG})`,
    "PARITY");
  ...
  call("fix below-floor -> ERR_PREMIUM_TOO_HIGH", MM, "fix-price", fixArgs(0, cLow, cLow, sigLow0), "(err u2005)");
  call("fix above-ceiling -> ERR_ABOVE_MAX_OUT", MM, "fix-price", fixArgs(0, cHigh, cHigh, sigHigh0), "(err u2009)");
  ...
  call("set-commit-efficiency by non-op -> ERR_NOT_AUTHORIZED", CLIENT, "set-commit-efficiency-bps",
    [uintCV(10000)], "(err u1011)");
  call("set-commit-efficiency too low -> ERR_BAD_CALIBRATION", DEPLOYER, "set-commit-efficiency-bps",
    [uintCV(4000)], "(err u1021)");
  call("set-commit-efficiency too high -> ERR_BAD_CALIBRATION", DEPLOYER, "set-commit-efficiency-bps",
    [uintCV(16000)], "(err u1021)");
  call("set-commit-efficiency u10900 -> ok", DEPLOYER, "set-commit-efficiency-bps", [uintCV(10900)], "(ok true)");
  evalc("native price @10900", "(get-native-price)", "NATIVE_10900");
  call("set-commit-efficiency back to u10000 -> ok", DEPLOYER, "set-commit-efficiency-bps",
    [uintCV(10000)], "(ok true)");
  ...
  call("open rfq5 (for expired)", CLIENT, "open-rfq", openArgs(), "(ok u5)");
  advance(7);
  call("fix after open-expiry -> ERR_EXPIRED", MM, "fix-price", fixArgs(5, cOk, cOk, sigOk5), "(err u2003)");
  ...
  call("reclaim rfq5 after expiry -> ok", DEPLOYER, "reclaim", ffArgs(5), `(ok u${SBTC_IN})`);
  ...
  // calibration scaling: price@10900 vs price@10000 * 1.09 (tolerance 1 for
  // single- vs double-division rounding)
  const scaled = (uintFrom(cap.NATIVE) * 10900n) / 10000n;
  const scaleDiff = uintFrom(cap.NATIVE_10900) - scaled;
  assert("calibration scales linearly", scaleDiff >= -1n && scaleDiff <= 1n,
    `(got ${uintFrom(cap.NATIVE_10900)}, want ~${scaled})`);
