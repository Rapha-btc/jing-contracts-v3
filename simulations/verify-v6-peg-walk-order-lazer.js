...
async function main() {
  ...
  tx("v6 initialize", call(DEP, "initialize", [contractPrincipalCV(DEP, MKT), sbtcT, wstxT, uintCV(MIN_SBTC), uintCV(MIN_STX), uintCV(1), uintCV(45)]), "(ok true)"); deploy(BUY_RUNG, src("jing-buy-stx-market-spread")); deploy(SELL_RUNG, src("jing-sell-stx-market-spread"));
  ...
  // ---- O4 (trace coverage): the x walk skips a maker: the taker's OWN resting ask ----
  // swap only refuses a resting position on the taker's deposit side, so a taker buying sats
  // may hold a resting ask; the walk reaches it first (best price) and skips it (maker == taker),
  // then fills the next ask. Exact rem == 0 is not needed for the skip arm this way.
  const WA = mk(90), WT = mk(92);
  const W_AMT = 2000n, W_NET = (W_AMT * PA(5n) + PPDF - 1n) / PPDF;
  tx("O4 fund WA sats", satsSend(WA, 2100n), "(ok true)"); tx("O4 fund WA stx", stxSend(WA, 1_000_000n), (v) => String(v).startsWith("(ok"));
  tx("O4 fund WT sats", satsSend(WT, 2100n), "(ok true)"); tx("O4 fund WT stx", stxSend(WT, grossFor(W_NET) + 2_000_000n), (v) => String(v).startsWith("(ok"));
  tx("O4 WT rests its own fixed ask 2000 at +3 bps (the best ask on the book)", depX(WT, W_AMT, PA(3n), null), `(ok u${W_AMT})`);
  tx("O4 WA fixed ask 2000 at +5 bps", depX(WA, W_AMT, PA(5n), null), `(ok u${W_AMT})`);
  const o4 = tx("O4 WT sells STX with limit +10 bps: the walk meets WT's own ask first and skips it, WA fills", swap(WT, grossFor(W_NET), PA(10n), false), (v) => String(v).startsWith("(ok"));
  ev("O4 WT's own ask intact (skipped)", depOfX(WT), `u${W_AMT}`);
  ev("O4 WA filled (partly or fully)", depOfX(WA), (v) => uintOf(v) < W_AMT);
  ...
  check("O4 exactly one match log (WA), none against WT's own ask", prints(s[o4.idx]).filter((p) => p.includes('(event "match")')).length, (v) => v === 1);
