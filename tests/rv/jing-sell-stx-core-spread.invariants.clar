;; ============================================================================
;; RENDEZVOUS INVARIANTS for jing-sell-stx-core-spread (pooled miner-band sell rung on markets v6)
;; ============================================================================
...
;; and pre-initializes it at 331.50 sats per STX, 20 bps, the cap read from the mock RFQ oracle at every push. RV fuzzes the
...
(define-public (rv-seat (on bool))
  (begin
    ;; a parked rung is not seated (the market's own wrappers refuse it too):
    ;; the seat's protection starts at the seat, it does not undo a park
    (asserts! (or (not on) (is-eq (contract-call? .v6-market get-token-y-parked current-contract) u0)) (err u6011))
    (try! (contract-call? .mock-jing-ladder set-band-y current-contract on))
    (if on
...
(define-read-only (invariant-no-stranded-proceeds)
  (<= (- (unwrap-panic (contract-call? .mock-ft get-balance current-contract)) (fold rv-proceeds-fold RV-ACCOUNTS u0))
...
(define-public (test-sync-idempotent)
  (match (sync)
    a (let (
        ...
        (h1 (var-get held-ustx))
        (w1 (var-get sats-accounted))
        (e1 (var-get epoch))
      ...
      (match (sync)
        b (if (and
            ...
            (is-eq h1 (var-get held-ustx))
            (is-eq w1 (var-get sats-accounted))
  ...
  (let ((amount (+ MIN_DEPOSIT (mod raw u1000000)))
    ...
    (match (deposit amount 0x)
      receipt (if (and
          ...
          (is-eq (get stx-paid receipt) u0)
          (is-eq (get sbtc-paid receipt) (- (unwrap-panic (contract-call? .mock-ft get-balance tx-sender)) before)))
        (ok true) (err u9110))
      error (ok false))))
