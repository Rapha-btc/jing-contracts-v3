...
(define-read-only (invariant-cleared-le-deposited-y)
  (let (
    (cycle (var-get current-cycle))
    (s (get-settlement (- cycle u1)))
  )
    (match s
      settlement
      (let (
        (totals (get-cycle-totals (- cycle u1)))
        (cleared (get token-y-cleared settlement))
      )
        (<= cleared (get total-token-y totals)))
      true)))
...
(define-read-only (invariant-cleared-le-deposited-x)
  (let (
    ...
    (match s
      ...
      (let (
        (totals (get-cycle-totals (- cycle u1)))
        (cleared (get token-x-cleared settlement))
      )
        (<= cleared (get total-token-x totals)))
      true)))
