;; ============================================================================
;; RENDEZVOUS INVARIANTS for markets-sbtc-stx-jing-v2
;; ============================================================================
;; Append-only block. The build script concatenates this onto the production
;; contract source to produce tests/rv/.build/markets-sbtc-stx-jing-v2.clar
...
;; STX market splits the balance check by side: production source has
;; hardcoded native `stx-transfer?` calls for y-side movements (token-y
;; is the wstx pseudo-token in production but transfers are real STX),
;; so mock-ft only tracks x-side flows and native STX tracks y-side.
(define-read-only (invariant-balance-eq-cycle-totals)
  (let (
    ...
    (mock-bal (unwrap-panic (contract-call? .mock-ft get-balance 'ST1PQHQKV0RJXZFY1DGX8MNSNYVE3VGZJSRTPGZGM.markets-sbtc-stx-jing-v2)))
    (stx-bal (stx-get-balance 'ST1PQHQKV0RJXZFY1DGX8MNSNYVE3VGZJSRTPGZGM.markets-sbtc-stx-jing-v2))
  )
    (and (is-eq mock-bal sum-x) (is-eq stx-bal sum-y))))

;; ============================================================================
;; INVARIANT 14 (v2-only): pending rebates are zero at rest
;;
;; pending-rebate-{x,y} are scratch vars set inside the atomic taker paths
;; (swap, reprice-or-swap crossing branch) and reset to u0 by
;; execute-settlement (lines ~1011-1012). Because those paths either settle
;; and reset in the same tx or revert whole, no committed state may ever
;; leave a nonzero pending rebate behind. A nonzero value at rest would mean
;; a code path escaped the atomic envelope -- rebate funds parked in the
;; contract outside cycle-totals accounting (the balance invariant above
;; would drift next).
;; ============================================================================

(define-read-only (invariant-pending-rebates-zero-at-rest)
