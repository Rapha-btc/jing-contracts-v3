...
| `simulations/verify-swap-router-sbtc-stx.js` | 158/158, [7f52982b](https://stxer.xyz/simulations/mainnet/7f52982b6b241e4f7a1bcc8863c74c96) | retail router `swap-router-sbtc-stx-jing` against the LIVE venues (DLMM v-2 pool, XYK pool via xyk-core direct, Velar pool 70) and markets v3 deployed as chavita against the live core (tip fork). W1 guards + partial fill, W2/W3 single venue both ways, W4 Jing + DLMM, W5 empty book / fallback, W7 four-leg split, W8 `get-taker-capacity` tight to the sat, W9 smart swaps (on-chain split: Jing to capacity, DLMM bins to the limit, XYK + Velar closed form; W9f four-venue happy path 0.7 BTC unsold u0; per-leg price vs limit). Fork Pyth storage is stale (oracle mid ~2x the AMM spot), so limits are taken on both sides of that gap on purpose. |
