...
| `simulations/verify-swap-router-sbtc-stx.js` | 165/165, [732ff37c](https://stxer.xyz/simulations/mainnet/732ff37cc7f8f595d29a38e3dec2fa09) | retail router `swap-router-sbtc-stx-jing` against the LIVE venues (DLMM v-2 pool, XYK pool via xyk-core direct, Velar pool 70) and markets v3 deployed as chavita against the live core (tip fork). W1 guards + partial fill, W2/W3 single venue both ways, W4 Jing + DLMM, W5 empty book / fallback, W7 four-leg split, W8 `get-taker-capacity` tight to the sat, W9 smart swaps (on-chain split: Jing to capacity, DLMM bins to the limit, XYK + Velar closed form; W9f four-venue happy path 0.7 BTC unsold u0 with the book walk boundary from both sides: a bid at the mid cleared, a bid inside the limit walked, a bid outside the limit untouched, then DLMM to its room and the spill-over on XYK + Velar; per-leg price vs limit). The fork's Pyth storage is stale, so the harness pins the market's `pyth-storage-v4 get-price` reads to today's Pyth mid (BTC $110,000 / STX $0.3710 = 337.29 sats/STX); the live AMMs trade ~1.6% off that, and every limit and maker price is set at realistic distances (bids at the mid, -0.5% inside a 1% taker limit, -2% outside it). |
| `simulations/verify-swap-router-v2-lazer.js` | 182/182, [9d8071e1](https://stxer.xyz/simulations/mainnet/9d8071e14ef206f23bb6bdd9073f7b01) | markets v4 on Pyth Lazer + router v2, market UNPATCHED on a tip fork with a real signed Lazer update (BTC feed 1 + STX feed 45); everything the v1 router harness proves, plus refresh-mid, freshness (stale fixture refused u1002 on refresh-mid, swap and a gated deposit), W9f spill-over on the real mid. See `README-markets-v4-lazer.md`. |
