...
(define-public (start-round
  ...
  (let (
    ...
    (try! (stx-transfer? deposit tx-sender current-contract))
...
(define-public (release
  ...
  (let (
    ...
    (try! (as-contract? ((with-stx per-video))
      (try! (stx-transfer? per-video current-contract recipient))
    ))
    (print {
  ...
  (let (
    ...
    (and
      ...
      (try! (as-contract? ((with-stx remaining))
        (try! (stx-transfer? remaining current-contract OWNER))
      ))
    )
    (print {
      event: "round-swept",
