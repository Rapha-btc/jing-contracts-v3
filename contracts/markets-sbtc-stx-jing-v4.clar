...
;; The keeper's entry: settles makers that crossed each other at the live
;; mid in one atomic step. There is no separate close any more; the book is
;; open between txs. swap and reprice-or-swap reach this same function.
...
(define-private (execute-settlement
  ...
  (let (
    ...
    (asserts! (not (var-get paused)) ERR_PAUSED)
    ;; Raw totals on both sides must meet the minimums, checked before the
    ;; range and size filters below. close-deposits used to do this on every
    ;; path; the post-filter check further down is skipped for a crossing
    ;; taker on purpose, so the walk can still fill makers that sit out of
    ;; range at the mid but inside the taker's limit.
    (asserts!
      (and
