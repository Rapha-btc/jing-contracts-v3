;; creator-escrow-stxer
;; Stxer mainnet-fork test variant of creator-escrow.
;;
;; Identical to the production contract EXCEPT all burn-block timing
;; gates are zeroed:
;;   REVIEW_WINDOW_BURN_BLOCKS  = u0
;;   CLAIM_GRACE_BURN_BLOCKS    = u0
;;   ROUND_BURN_BLOCKS          = u0
;;
;; Stxer executes every step at a single burn-block height, so any
;; non-zero gate would make submit / release / sweep unreachable in one
;; simulation. With everything zeroed, the full lifecycle (start-round
;; -> submit -> release -> sweep -> next round) can be exercised in a
;; single sim. The veto path (which requires `now < review-ends-at`)
;; is unreachable here -- a separate stxer variant with REVIEW > 0 is
;; needed for veto/lift-veto sims.
...
(define-constant REVIEW_WINDOW_BURN_BLOCKS u0)
;; STXER OVERRIDE: 0 (mainnet: u288).
...
;; ---------------------------------------------------------------
;; Public functions (identical to production)
;; ---------------------------------------------------------------
