;; AUTO-DERIVED TESTABLE COPY - DO NOT EDIT BY HAND.
;; Identical to contracts/vault-sbtc-stx-v2.clar except: (1) every
;; 'SPV9K21TBFAK4KNRJXF5DFP8N7W46G4V9RCJDC22. prefix is rewritten to a
;; relative reference so the manifest deploys it against the local jing
;; stack, and (2) the four Bitflow venue principals are rewritten to the RV
;; mock venues (tests/rv/mock-xyk-*, mock-dlmm-*) because the real wstx
;; facade's is-standard asserts reject simnet's mixed ST/SM principals -
;; the same substitution the RV harness uses. Real-venue integration truth
;; lives in simulations/verify-vault-sbtc-stx-v2.js (stxer mainnet fork).
;; The sync guard in tests/vault-sbtc-stx-v2-sync.test.ts re-derives this
;; transformation from the canonical source and fails if this file drifts.
...
(define-constant JING-MARKET .markets-sbtc-stx-jing-v2)
(define-constant JING-CORE .jing-core-v3)
(define-constant JING-VAULT-AUTH .jing-vault-auth)

(define-constant XYK_CORE .mock-xyk-core)
(define-constant XYK_POOL_SBTC_STX .mock-xyk-pool)

(define-constant DLMM_ROUTER .mock-dlmm-router)
(define-constant DLMM_POOL_STX_SBTC .mock-dlmm-pool)
...
;; (the fee is zero only when the feeds were already refreshed). Current
;; mainnet fee is 1 uSTX per feed (2 feeds on this market); u10 leaves
;; governance headroom. The vault must hold at least this much free STX for
;; the jing-deposit / jing-swap / crossing-reprice paths to work.
...
(define-public (execute-jing-reprice
  ...
  (let (
    ...
    (try! (verify-and-consume msg-hash sig expiry))
    (let ((result (if (is-eq side ASSET_WSTX)
        (try! (as-contract? ((with-stx (+ rebate PYTH_FEE_BUDGET)))
