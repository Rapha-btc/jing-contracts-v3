...
;; markets-sbtc-stx-jing-v4. Many users, one maker slot: the contract rests
...
(define-constant MARKET 'SPV9K21TBFAK4KNRJXF5DFP8N7W46G4V9RCJDC22.markets-sbtc-stx-jing-v4)
...
(define-read-only (market-size)
  (+
    (contract-call? 'SPV9K21TBFAK4KNRJXF5DFP8N7W46G4V9RCJDC22.markets-sbtc-stx-jing-v4
      get-token-x-deposit
      (contract-call? 'SPV9K21TBFAK4KNRJXF5DFP8N7W46G4V9RCJDC22.markets-sbtc-stx-jing-v4
        get-current-cycle
      )
...
(define-public (initialize (cents uint))
  (begin
    ...
    (let ((p (/ PRICE_NUMERATOR cents)))
      ...
      (try! (contract-call? LADDER register SIDE cents))
      (ok (print {
        event: "initialize",
        side: SIDE,
        sats-per-stx-cents: cents,
        price: p,
      }))
    )
  )
        ...
        (and
          ...
          (begin
            ...
            (print {
              event: "epoch-closed",
              epoch: current-epoch,
              final-proceeds-index: new-proceeds,
            })
