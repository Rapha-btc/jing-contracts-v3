  ...
  (let (
    ...
    (if (and (is-eq existing u0) (>= (len depositors) MAX_DEPOSITORS))
      (let (
        ...
        (try! (contract-call? .jing-core log-deposit-y tx-sender amount amount
      ...
      (begin
        ...
        (try! (contract-call? .jing-core log-deposit-y tx-sender (+ existing amount)
  ...
  (let (
    ...
    (if (and (is-eq existing u0) (>= (len depositors) MAX_DEPOSITORS))
      (let (
        ...
        (try! (contract-call? .jing-core log-deposit-x tx-sender amount amount
      ...
      (begin
        ...
        (try! (contract-call? .jing-core log-deposit-x tx-sender (+ existing amount)
...
(define-public (cancel-token-y-deposit
  ...
  (let (
    ...
    (try! (contract-call? .jing-core log-refund-y caller amount cycle (var-get token-x)
      tok-y
    ))
    (ok amount)
  )
...
(define-public (cancel-token-x-deposit
  ...
  (let (
    ...
    (try! (contract-call? .jing-core log-refund-x caller amount cycle tok-x
      (var-get token-y)
    ))
    (ok amount)
  )
...
(define-public (set-token-y-limit (limit-price uint))
  (begin
    ...
    (map-set token-y-deposit-limits tx-sender limit-price)
    (try! (contract-call? .jing-core log-set-limit-y tx-sender limit-price
...
(define-public (set-token-x-limit (limit-price uint))
  (begin
    ...
    (map-set token-x-deposit-limits tx-sender limit-price)
    (try! (contract-call? .jing-core log-set-limit-x tx-sender limit-price
      (var-get token-x) (var-get token-y)
    ))
    (ok true)
  )
  ...
  (let (
    ...
    (if (< (* amount BPS_PRECISION) (* total-token-y MIN_SHARE_BPS))
      (begin
        ...
        (try! (contract-call? .jing-core log-small-share-roll-y depositor cycle amount
          (var-get token-x) (var-get token-y)
        ))
        (ok true)
      )
  ...
  (let (
    ...
    (if (< (* amount BPS_PRECISION) (* total-token-x MIN_SHARE_BPS))
      (begin
        ...
        (try! (contract-call? .jing-core log-small-share-roll-x depositor cycle amount
          (var-get token-x) (var-get token-y)
        ))
        (ok true)
      )
  ...
  (let (
    ...
    (if (> clearing limit)
      (begin
        ...
        (try! (contract-call? .jing-core log-limit-roll-y depositor cycle amount limit
          clearing (var-get token-x) (var-get token-y)
        ))
        (ok true)
      )
      (ok true)
  ...
  (let (
    ...
    (if (< clearing limit)
      (begin
        ...
        (try! (contract-call? .jing-core log-limit-roll-x depositor cycle amount limit
          clearing (var-get token-x) (var-get token-y)
        ))
        (ok true)
      )
...
(define-public (close-deposits)
  (let (
    ...
    (try! (contract-call? .jing-core log-close-deposits cycle stacks-block-height
...
(define-public (close-and-settle-with-refresh
  ...
  (begin
    (try! (close-deposits))
    (settle-with-refresh vaa pyth-storage pyth-decoder wormhole-core tx-trait
      tx-name ty-trait ty-name
    )
  )
)
...
(define-public (cancel-cycle)
  (let (
    ...
    (advance-cycle)
    (try! (contract-call? .jing-core log-cancel-cycle cycle merged-x merged-y
      (var-get token-x) (var-get token-y)
    ))
    (ok true)
  )
...
(define-private (execute-settlement
  ...
  (let (
    ...
    (asserts! (< (get conf feed-data) (/ oracle-price MAX_CONF_RATIO))
      ERR_PRICE_UNCERTAIN
    )
    ...
    (let (
      ...
      (if (> token-y-fee u0)
        (try! (as-contract? ((with-ft (contract-of ty-trait) ty-name token-y-fee))
          (try! (contract-call? ty-trait transfer token-y-fee current-contract
      ...
      (try! (contract-call? .jing-core log-settlement cycle oracle-price oracle-price
        token-x-clearing token-y-clearing token-x-unfilled token-y-unfilled
        token-x-fee token-y-fee token-x-is-binding (var-get token-x)
        (var-get token-y)
      ))
      (ok true)
    )
...
(define-private (distribute-to-token-y-depositor
  ...
  (let (
    ...
    (try! (contract-call? .jing-core log-distribute-y-depositor depositor cycle
...
(define-private (distribute-to-token-x-depositor
  ...
  (let (
    ...
    (if (> my-token-y-received u0)
      (try! (as-contract?
        ((with-ft (contract-of tt) (get name unwrapped) my-token-y-received))
        (try! (contract-call? tt transfer my-token-y-received current-contract
          depositor none
        ))
      ))
      true
    ...
    (try! (contract-call? .jing-core log-distribute-x-depositor depositor cycle
  ...
  (let (
    ...
    (if (> token-y-dust u0)
      (try! (as-contract? ((with-ft (contract-of ty-trait) ty-name token-y-dust))
        (try! (contract-call? ty-trait transfer token-y-dust current-contract
          (var-get treasury) none
        ))
      ))
      true
    ...
    (try! (contract-call? .jing-core log-sweep-dust acc-token-x-rol acc-token-y-rol
...
(define-public (initialize
  ...
  (begin
    ...
    ;; initialize with attacker-chosen tokens/feed and write events to
